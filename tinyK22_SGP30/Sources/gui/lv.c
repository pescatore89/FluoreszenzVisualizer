 ...
 * This function is required only when LV_VDB_SIZE == 0 in lv_conf.h*/
static void ex_disp_map(int32_t x1, int32_t y1, int32_t x2, int32_t y2,
		const lv_color_t * color_p) {
	/*The most simple case (but also the slowest) to put all pixels to the screen one-by-one*/
	int32_t x;
	int32_t y;
...
/* Write a pixel array (called 'map') to the a specific area on the display
 * This function is required only when LV_VDB_SIZE == 0 in lv_conf.h*/
static void ex_disp_fill(int32_t x1, int32_t y1, int32_t x2, int32_t y2,
		lv_color_t color) {
	/*The most simple case (but also the slowest) to put all pixels to the screen one-by-one*/
	int32_t x;
	int32_t y;
...
static bool keyboard_read(lv_indev_data_t *data) {
	...
	if (keyData == (LV_BUTTON_SW0 | LV_MASK_PRESSED)) {
		...
		/* Switch 4 (LH)*/
	} else if (keyData == (LV_BUTTON_SW3 | LV_MASK_PRESSED)) {
		...
		/* Switch 5 (DOWN)*/
	} else if (keyData == (LV_BUTTON_SW4 | LV_MASK_PRESSED)) {
		data->key = LV_GROUP_KEY_NEXT;
		data->state = LV_INDEV_STATE_PR;
	} else if (keyData == (LV_BUTTON_SW4 | LV_MASK_RELEASED)) {
		data->key = LV_GROUP_KEY_NEXT;
		data->state = LV_INDEV_STATE_REL;
